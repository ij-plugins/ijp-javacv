...
import ij_plugins.javacv.mcc.Utils.{percentileMinMax, scalar}
...
@main
def runCV(fileName: String): Unit =

  val file = new File(fileName)
//  val file = new File("data/IMG_0903_025p-crop.png")
  val im1 = OpenCVUtils.load(file)
  printInfo(im1, "im1")
  OpenCVUtils.show(im1, "Source")

  val (minV, maxV) = percentileMinMax(im1, 0.01)
  ...
  val beta  = -minV * alpha
  im1.convertTo(im2, CV_8U, alpha, beta)
  OpenCVUtils.show(im2, "Source Enhanced Brightness Range")

  val detector = CCheckerDetector.create()

  val hasChart = detector.process(im2, MCC24)
  ...
  if hasChart then
    ...
    OpenCVUtils.show(im2, "Detected ColorChecker tiles")
  else
